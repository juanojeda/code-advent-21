...
type Method = BaseMethod<PositionWithAim>;
type MethodStore = BaseMethodStore<PositionWithAim>;

const up: Method = ({ aim, x, y }, val) => ({ x, y, aim: aim - val });

const down: Method = ({ aim, x, y }, val) => ({
  x,
  y,
  aim: aim + val
});

const forward: Method = ({ x, y, aim }, val) => ({
  x: x + val,
  y: y + aim * val,
  aim
});

const getPositionProduct = ({ x, y }: PositionWithAim): number => x * y;

const createSub =
  (methods: MethodStore) =>
  (
    instructions: readonly ErrorOr<Instruction>[],
    initPos: PositionWithAim
  ): PositionWithAim =>
    instructions.reduce(
      (prevPos, i) =>
